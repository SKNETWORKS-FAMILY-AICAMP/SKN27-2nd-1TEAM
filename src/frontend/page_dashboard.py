...
def render():
    ...
    with tab1:
        ...
        with col_pie:
            ...
            seg.columns = ['등급', '수']
            fig = px.pie(seg, names='등급', values='수',
                         color='등급',
                         color_discrete_map={
                             '🔴 High Risk': '#F44336',
                             '🟡 Warning'  : '#FFC107',
        ...
        with col_bar:
            ...
            if 'contract' in df_pred.columns:
                ...
                fig.update_layout(margin=dict(t=20,b=0), 
                                  xaxis_title='계약 유형', yaxis_title='고객 수',
                                  legend_title='위험 등급')
                st.plotly_chart(fig, use_container_width=True)
        ...
        with ch1:
            ...
            if 'payment_method' in df_pred.columns:
                ...
                fig.update_layout(xaxis_title='결제 방식', yaxis_title='고객 수',
                                  margin=dict(t=40,b=0), xaxis_tickangle=-20)
                st.plotly_chart(fig, use_container_width=True)
        ...
        with ch2:
            ...
            if 'internet' in df_pred.columns:
                ...
                fig.update_layout(xaxis_title='인터넷 서비스', yaxis_title='고객 수',
            ...
            if 'tenure_months' in df_pred.columns:
                fig = px.scatter(df_pred, x='tenure_months', y='churn_prob',
                                 ...
                                 color_discrete_map={
                                     ...
                                     '🟡 Warning'  : '#FFC107',
                                     '🟢 Safe'     : '#4CAF50'},
                                 title='이용기간별 이탈 확률 분포', height=300,
                                 opacity=0.7)
                fig.update_layout(xaxis_title='이용 기간 (월)',
                                  yaxis_title='이탈 확률',
                                  margin=dict(t=40,b=0))
        ...
        with ch4:
            ...
            if 'monthly_charges' in df_pred.columns:
                ...
                fig.update_layout(xaxis_title='월 요금 구간', yaxis_title='고객 수',
                                  margin=dict(t=40,b=0))
                st.plotly_chart(fig, use_container_width=True)
        ...
        with col_f1:
            grade_f = st.selectbox("위험 등급 필터",
                                   ["전체", "🔴 High Risk", "🟡 Warning", "🟢 Safe"])
        with col_f2:
            search = st.text_input("고객 ID 검색")

        df_show = df_pred.copy().sort_values('churn_prob', ascending=False)
        if grade_f != "전체":
        ...
        # 표시할 컬럼 정리
        df_table = df_show[['customer_id','churn_prob','위험등급','contract',
                             'internet','monthly_charges','tenure_months',
                             'payment_method','predicted_at']].copy()
        df_table['churn_prob'] = (df_table['churn_prob'] * 100).round(1).astype(str) + '%'
        df_table.columns = ['고객 ID','이탈 확률','위험 등급','계약 유형',
                             '인터넷','월 요금($)','이용기간(월)','결제 방식','예측 시간']

        st.dataframe(df_table.reset_index(drop=True), use_container_width=True)

        # ── 비즈니스 임팩트 ───────────────────────────
        st.markdown("---")
        st.markdown("#### 💰 비즈니스 임팩트 요약")

        avg_cltv   = 4149
        high_risk  = len(df_pred[df_pred['위험등급'] == '🔴 High Risk'])
        warning    = len(df_pred[df_pred['위험등급'] == '🟡 Warning'])
        total_risk = high_risk + warning
        potential_loss = total_risk * avg_cltv
        defense_30 = int(total_risk * 0.3 * avg_cltv)

        b1, b2, b3 = st.columns(3)
        b1.metric("이탈 위험 고객 (분석 기준)", f"{total_risk:,}명",
                  delta=f"High Risk {high_risk}명 포함", delta_color='inverse')
        b2.metric("예상 손실 CLTV",  f"${potential_loss:,.0f}")
        b3.metric("30% 방어 시 절약", f"${defense_30:,.0f}",
                  delta="리텐션 캠페인 권장", delta_color='normal')
